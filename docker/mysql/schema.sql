...
CREATE VIEW sparschwein.currentPeriod AS SELECT ID, Budget, (SELECT COALESCE(SUM(Total), 0) FROM sparschwein.purchases pu WHERE pu.Timestamp BETWEEN pe.Start AND pe.End) AS Spent FROM sparschwein.periods pe WHERE End > NOW() AND Start < NOW() ORDER BY Start DESC LIMIT 1;
